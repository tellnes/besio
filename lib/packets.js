...
IncomingPacket.prototype.handleHandshake = function() {
  ...
  if (!server) {
    client.onAuthorized(arr)

  } else {
    handshakeData = client.handshakeData(arr[0])

    server.authorize(handshakeData, function(err, authorized, authorizedData) {
      if (!err && !authorized) {
        err = new Error('unauthorized');
      }

      if (err) {
        debug('Client authorization failed; %s', err)
        client.error(err);
        client.end();

      } else {
        var data = [server.options['heartbeat timeout'], server.options['heartbeat interval']]
        var packet = new OutgoingPacket(client, packets.TYPE_HANDSHAKE, binson.calculate(data));
        packet.writeBinson(data);
        client.sendPacket(packet);

        client.onAuthorized(data, authorizedData)
      }
    });
  }
};
...
IncomingPacket.prototype.handleStream = function() {
  ...
  var id = packet.readBuffer(16)
  var type = packet.readUInt8();

  var client = packet.client;

  var stream = client.streams[id.toString('hex')]
  ...
  case BesioStream.TYPE_CREATE:
    debug('Handle stream create packet (%s)', stream.id_hex)
    stream.createdRemote = true
    if (stream.paused) {
      stream.paused = false;
      stream.pause();
    }
  ...
  case BesioStream.TYPE_END:
    debug('Handle stream end packet (%s)', stream.id_hex)
    stream.emit('end');
    stream.destroy();
    break;

  case BesioStream.TYPE_PAUSE:
    debug('Handle stream pause packet (%s)', stream.id_hex)
    stream.remotePaused = true;
    break;

  case BesioStream.TYPE_RESUME:
    debug('Handle stream resume packet (%s)', stream.id_hex)
    stream.remotePaused = false;
    stream.writeQueue();
    break;

  case BesioStream.TYPE_DATA:
    debug('Handle stream data packet (%s)', stream.id_hex)
...
IncomingPacket.prototype.handleHeartbeat = function() {
  this.client.clearHeartbeatTimeout()
  this.client.setHeartbeatTimeout()
}
