...
Client.prototype.connect = function() {
  ...
  if (options.http) {
    ...
    var socket = (secure ? tls : net).connect(options, onConnect)
    socket.write(binson.encode(pkg.version))
...
Client.prototype.onAuthorized = function(info, authorizedData) {
  ...
  if (this.server) {
    this.server.emit('client', this, authorizedData)
  }

  this.emit('connect')


  this.heartbeatTimeoutValue = info[0] * 1000
  this.heartbeatIntervalValue = info[1] * 1000
