...
function BesioStream(client) {
  ...
  this.log = client.log;
  this.client = client;
  this.id = client.streams.length;
  client.streams.push(this);
...
BesioStream.prototype.log = winston;
...
BesioStream.prototype.createPacket = function(type, length) {
  var packet = new packets.OutgoingPacket(packets.TYPE_STREAM, length + 5);
...
BufferStream.prototype.readBuffer = function(size) {
  ...
  if (this.buffers[0].length >= (size-this.index)) { // can slice
    ...
    while(offset < size) {
      ...
      read = Math.min((source.length - index), size);
