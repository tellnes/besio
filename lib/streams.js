...
binson.register(0, {
  ...
  decode: function (data) {
    ...
    var stream = new BesioStream(packets.currentClient, data[0])
    stream.readable = data[1];
    stream.writable = data[2];

    var packet = stream.createPacket(BesioStream.TYPE_CREATE, 0)
...
function BesioStream(client, id) {
  Stream.call(this);

  this.client = client;

  this.id = id || uuid.v4('binary')
  this.id_hex = this.id.toString('hex')
  client.streams[this.id_hex] = this

  this._writeQueue = [];

  this.createdRemote = false
...
BesioStream.prototype.write = function(data) {
  if (!this.createdRemote || this.remotePaused) {
...
BesioStream.prototype._write = function(data) {
  ...
  debug('Sending stream data packet (%s)', this.id_hex)
...
BesioStream.prototype.pause = function() {
  ...
  if (!this.createdRemote) {
    return;
  }

  debug('Sending stream pause packet (%s)', this.id_hex)
...
BesioStream.prototype.resume = function() {
  ...
  debug('Sending stream resume packet (%s)', this.id_hex)
...
BesioStream.prototype.end = function(data) {
  ...
  debug('Sending stream end packet (%s)', this.id_hex)
...
BesioStream.prototype.destroy = function() {
  delete this.client.streams[this.id_hex]
  this.readable = false;
  this.writable = false;
...
BesioStream.prototype.createPacket = function(type, length) {
  var packet = new packets.OutgoingPacket(this.client, packets.TYPE_STREAM, length + 17)

  this.id.copy(packet, packet.index)
  packet.index += 16
